...
export function flowLocators(page: Page) {
  return {
    ...
    promptBox: page.locator('[role="textbox"][contenteditable="true"], .ProseMirror[contenteditable="true"]'),
    ...
    submitButton: page.locator('button[aria-label="Start generation"]').or(
      page.locator("button").filter({ hasText: "Create" }).filter({ hasText: /arrow_forward/ })
    ),
